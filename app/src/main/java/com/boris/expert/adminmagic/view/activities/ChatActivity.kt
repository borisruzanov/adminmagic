...
import com.boris.expert.adminmagic.utils.FileUtils
...
class ChatActivity : BaseActivity(), View.OnClickListener {
    ...
    private var name = ""
    private var isAdminReplied = false
    private var userId = ""
    ...
    private fun initViews() {
        ...
        val wrapContentLinearLayoutManager =
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            ...
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                data?.let { oldData ->
                    ...
                    val realPath: String? = FileUtils.getRealPathFromURI(context, path!!)
                    startLoading(context)
                    uploadImageOnFirebaseStorage("ChatImages/$userId",realPath!!,object :UploadFileCallback{
                        override fun onSuccess(url: String) {
                            dismiss()
                            Toast.makeText(context,"Image uploaded successfully!",Toast.LENGTH_SHORT).show()
                            selectedImageServerUrl = url
                            messageType = "image"
                            sendMessage(messageType)
                            messageType = "text"
                        }
        ...
        if (message.isNotEmpty() || selectedImageServerUrl.isNotEmpty()) {
            ...
            databaseReference.child(Constants.supportChat).child(supportTicket!!.id).child(id)
                .setValue(messageItem).addOnSuccessListener {
                    ...
                    if (isAdminReplied && supportTicket!!.status == "open") {
